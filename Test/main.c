...
typedef int (*child_func_pointer_t)(void*);

pid_t fork_child_in_function(child_func_pointer_t child_func, const char child_name[], void *arg)
{
...
int wait_child(pid_t child_pid, const char child_name[])
{
    int child_status;
...
int fork_and_wait_child(child_func_pointer_t child_func, const char child_name[], void *arg)
{
    pid_t child_pid = fork_child_in_function(child_func, child_name, arg);
    return wait_child(child_pid, child_name);
}
